...
def main():
    base_dir = os.path.join('..', 'log', 'test_randomsearchVTHE_ONE')

    tested_file = os.path.join('..', 'log', 'test_randomsearchVTHE_ONE', 'tested_combinations_THE_ONE.json')
    processed_file = os.path.join('..', 'log', 'test_randomsearchVTHE_ONE', 'processed_combinations_THE_ONE.json')

    for _ in range(30):
        progress_manager = ProgressManager(tested_file, processed_file)
        unprocessed_combinations = progress_manager.get_unprocessed_combinations(batch_size=5)
        #unprocessed_combinations = [[10, 10, 25, 200000, 0.05, 0.3, 0.6],[5, 5, 25, 200000, 0.05, 0.3, 0.6]]
        if len(unprocessed_combinations) == 0: break
